...
const CardBoxStyle = ({ label, id, published, link, handleUpdate }: dataType) => {
  ...
  return (
    <div className={Styles.card} >
      <div className={Styles.labelText}>
       ...
       {showHidePublish && ( <Tooltip title="Update Status">
          <IconButton onClick={() => { 
            typeof(published) != "string" ? handleUpdate( published, id) : ""
           }}>
          <div className={Styles.published}  >{published ? <div className={Styles.publisedPublised}>Published</div> : <div className={Styles.publisedUnPublised}>Draft</div> }</div>
          </IconButton>
        </Tooltip>)}
      </div>
