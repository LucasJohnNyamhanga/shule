...
import { form, questionFormat, review, subject, topic, question } from '@prisma/client'
...
const EditNotes = ({ question, forms, subjects, questionFormat }: InferGetServerSidePropsType<typeof getServerSideProps> ) => {
  ...
  let handleSelectFormat = (value:string) => {
    ...
    setQuestionDetails({ ...questionDetails, questionFormatId: parseInt(value) });
  ...
  let handleContentQuestion = (data: string) => {
    let convertedData = data.replaceAll(`img`, `Image layout="fill" objectFit="cover"`);
          ...
          case "True or False":
              if(answer1.answer == "True" || answer1.answer == "False"){
                {let answers = [];
                  ...
                  let answer2:string = String(!answer1.answer).replace(/^./, str => str.toUpperCase());
                  answers.push({
                    answer: answer2,
                    valid: false,
                  });
    ...
    <div className={Styles.innerContainer}>
        <div className={Styles.rightInnercontainerBody}>
            <div className={Styles.mainMain}>
                ...
                {formatName(questionDetails.questionFormatId) == "True or False" &&
                  <div className={Styles.question}>
                    <div className={Styles.questionFormatHeader}>{formatName(questionDetails.questionFormatId)} Question Format</div>
                    { showQuestion && <div>Question</div>}
                    <CkEditor  onReadyToStart={handleOnReady} content={handleContentQuestion} dataCk={questionDetails.question}  />
                    <SelectMiu displayLabel="Correct Answer" show={true} forms={trueAndFalse} handlechange={handleAnswerValid} value={answer1.answer} />
                    { showQuestion && <div>More Answer Details</div>}
                    <CkEditor  onReadyToStart={handleOnReady} content={handleContentAnswerDetails} dataCk={questionDetails.answerDetails} />
                  </div>
                }
                {formatName(questionDetails.questionFormatId) == "Multiple Choice" &&
                  <div className={Styles.question}>
                    <div className={Styles.questionFormatHeader}>{formatName(questionDetails.questionFormatId)} Question Format</div>
                    { showQuestion && <div>Question</div>}
                    <CkEditor  onReadyToStart={handleOnReady} content={handleContentQuestion} dataCk={questionDetails.question}  />
                    { showQuestion && <div>Valid Answer</div>}
                    <CkEditor  onReadyToStart={handleOnReady} content={handleContentAnswerValid} dataCk={answer1.answer} />
                    { showQuestion && <div>Option 1</div>}
                    <CkEditor  onReadyToStart={handleOnReady} content={handleContentAnswerInvalid1} dataCk={answer2.answer} />
                    { showQuestion && <div>Option 2</div>}
                    <CkEditor  onReadyToStart={handleOnReady} content={handleContentAnswerInvalid2} dataCk={answer3.answer} />
                    { showQuestion && <div>Option 3</div>}
                    <CkEditor  onReadyToStart={handleOnReady} content={handleContentAnswerInvalid3} dataCk={answer4.answer} />
                    { showQuestion && <div>More Answer Details</div>}
                    <CkEditor  onReadyToStart={handleOnReady} content={handleContentAnswerDetails} dataCk={questionDetails.answerDetails} />
                </div>
                }
            ...
            <div className={Styles.mainLeft}>
              ...
              {showQuestion && <>
                  <SelectMiu displayLabel="Question Format" show={true} forms={formatOptions} handlechange={handleSelectFormat} value={questionDetails.questionFormatId.toString()} />
                  <SelectMiu displayLabel="Select Subject" show={true} forms={subjectOptions} handlechange={handleSelectSubject} value={subjectValidation.subjectId} /><SelectMiu displayLabel="Select Form" show={true} forms={formOptions} handlechange={handleSelectForm} value={subjectValidation.formId} />
                  {hideShowTopic && <SelectMiu displayLabel="Select Topic" show={true} forms={topicOptions} handlechange={handleSelectTopic} value={subjectValidation.topicId} />}
                  {hideShowReview && <SelectMiu displayLabel="Select Review" show={true} forms={reviewOptions} handlechange={handleSelectReview} value={questionDetails.reviewId == 0 ? '' : questionDetails.reviewId.toString()} />}
                </>}
            </div>
          </div>
        { showQuestion && <div onClick={handleCreateTopic} className={Styles.imageSelect}>Update Question</div>}
   </div>
</div>
  )
