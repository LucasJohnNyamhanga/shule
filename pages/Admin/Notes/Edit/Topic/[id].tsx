...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  let Id = parseInt(id);
    ...
    const topicServer = await prisma.topic.findUnique({
      where: {
        id: Id,
      },
      select: {
...
const EditSubject = ({ topic, formsList, subjects }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    useEffect(() => {
      ...
      let sendToDatabase = () => {
          ...
          axios({
              method: 'post',
              url: 'http://localhost:3000/api/updateTopic',
              data: subjectDetails
        ...
        <div className={Styles.innerContainer}>
            <div className={Styles.rightInnercontainerBody}>
                <div className={Styles.mainMain}>
                    ...
                    <InputTextMui label='Topic Name' content={subjectDetails.topicName}  name='topicName' handleChange={handleTextInput} />
                    <InputTextMui  label='Topic Definition'  content={subjectDetails.topicDefinition} name='topicDefinition' handleChange={handleTextInput} />
                </div>
                <div className={Styles.mainLeft}>
                    <div className={Styles.formHeader}>
                        Topic Relations.
                    </div>
                        <SelectMiu displayLabel="Select Subject" value={subjectDetails.subjectId} show={true} forms={subjectOptions} handlechange={handleSelectSubject} />
                        <SelectMiu displayLabel="Select Form" value={subjectDetails.formId} show={true} forms={formOptions} handlechange={handleSelectForm} />
                    </div>
            </div>
            <div onClick={handleCreateTopic} className={Styles.imageSelect}>Update Topic</div>
       </div>
    </div>
      )
    }
