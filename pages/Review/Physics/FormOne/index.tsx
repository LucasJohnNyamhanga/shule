...
const Index = ({ topics, note }: InferGetStaticPropsType<typeof getStaticProps>) => {
  ...
  if (note[0].review ==  null || note[0].review == "undefined") {
    return <div className={Styles.notFound} >Reviews for ${note[0].topicName} topic will be available soon.</div>
  }
      ...
      <div className={Styles.innerContainer}>
        ...
        <div className={Styles.rightInnercontainerBody}>
          <div className={Styles.mobile}>
            <Drawer textHeader={'LIST OF TOPICS'} topic={topics} active={note[0].id} link={'Review'}/>
          </div>
            ...
            <div className={Styles.modal}>
            {
                  note.map((topic:any) => (
                    topic.review.map((review:review) => (
                      <Modal key={review.id} name={review.name} id={review.id} subject={note[0].subject.subjectName} topic={note[0].topicName} form={note[0].form.formName} />
                    ))
                  ))
                }
            </div>
